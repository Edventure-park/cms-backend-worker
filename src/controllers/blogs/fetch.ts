...
export const getAllBlogs = async (c: Context) => {
  try {
    ...
    const postType = c.req.query('postType')?.toLowerCase();
    ...
    if (postType) conditions.push(eq(blogs.postType, postType));
    ...
    if (isApproved !== undefined) conditions.push(eq(blogs.isApproved, isApproved === 'true'));
